...
def paginated_success_response(data=None, status=status.HTTP_200_OK, page_number=1, has_next_page=True, total_pages=0):
    if data is None:
        return Response(
            {"success": True, "page_number": page_number, "has_next_page": has_next_page, "total_pages": total_pages},
            status=status,
        )
    return Response(
        {
